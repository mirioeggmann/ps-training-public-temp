...
public class IOUTransferFlowTests {
    ...
    public void flowReturnsCorrectlyFormedPartiallySignedTransaction() throws Exception {
        ...
        assertTrue(ptx.getTx().getInputs().size() == 1);
        assertTrue(ptx.getTx().getOutputs().size() == 1);
        assertTrue(ptx.getTx().getOutputs().get(0).getData() instanceof IOUState);
        assertTrue(ptx.getTx().getInputs().get(0).equals(new StateRef(stx.getId(), 0)));

        IOUState outputIOU = (IOUState) ptx.getTx().getOutput(0);
        Command command = ptx.getTx().getCommands().get(0);

        assertTrue(command.getValue().equals(new IOUContract.Commands.Transfer()));
    ...
    public void flowCanOnlyBeRunByCurrentLender() throws Exception {
        ...
        assert (e != null);
        assert e.getMessage().equals("java.lang.IllegalArgumentException: This flow must be run by the current lender.");
    }
    ...
    public void iouCannotBeTransferredToSameParty() throws Exception {
        ...
        assert(e != null);
        assert e.getMessage().contains("Contract verification failed: Failed requirement: The lender property must change in a transfer.");
    }
    ...
    public void flowReturnsTransactionSignedByAllPartiesAndNotary() throws Exception {
        ...
        try {
            ...
            System.out.println(exception.getMessage());
            // Exception should not have occurred
            assert(false);
        }
    }
}